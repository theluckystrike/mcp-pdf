...
test.skip("stdout carries JSON-RPC only, including on a file the reader cannot parse", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
...
test.skip("every tool description is non-empty, single-paragraph, and within the hard ceiling", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
...
test.skip("a tool that takes a file path or a URL opens with an imperative sentence", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
...
test.skip("corrupt register: quarantined byte-for-byte, and the pdf work still succeeds", async (t) => {
  const b = box();
...
test.skip("cap: a merge over the free file limit writes no file at all", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
