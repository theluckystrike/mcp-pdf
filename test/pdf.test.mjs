...
test.skip("merge: page counts add up and the sources are untouched", async () => {
...
test.skip("split: ranges including an open-ended one, and pages left out are reported", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("split: a range past the end is refused and no part is written", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("pages: extraction keeps the order written, duplicates included", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("rotate: the rotation is on the page after a reload, and adds to what was there", async () => {
  const { dir, dataHome } = sandbox();
  const src = await makePdf(join(dir, "src.pdf"), 3, "s");
  const c = client({ dataHome, key: proKey() });
  try {
...
test.skip("stamp: PAID reaches the content stream and the source is unchanged", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("stamp: custom text and colour on Pro, page selection honoured", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("stamp: characters a built-in font cannot carry are removed and reported", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("reorder: a permutation is applied, an incomplete order is refused", async () => {
  const { dir, dataHome } = sandbox();
  const src = await makePdf(join(dir, "src.pdf"), 3, "s");
  const c = client({ dataHome, key: proKey() });
  try {
...
test.skip("encrypted: every writing tool refuses the file with a reason", async () => {
  const { dir, dataHome } = sandbox();
...
test.skip("input guards: missing file, a non-PDF and a directory are all refused", async () => {
  const { dir, dataHome } = sandbox();
  const { writeFileSync } = await import("node:fs");
