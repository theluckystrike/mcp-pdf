...
test.skip("an out_path that is also an input is refused, with and without overwrite", async () => {
...
test.skip("stamp text: a newline separates words, a Polish stroke is transliterated, CJK is removed", async () => {
...
test.skip("font_size is bounded and an overlong stamp says it runs off the page", async () => {
...
test.skip("a PDF/A claim is reported, and every write path says the claim no longer holds", async () => {
...
test.skip("a 200 MB input is refused before it is read, and a 2000-page file is handled", async () => {
...
test.skip("a truncated PDF and a wrong header are both refused with nothing written", async () => {
...
test.skip("object streams and a cross-reference stream are read, written and re-read", async () => {
...
test.skip("ranges: 0, 3-1, a-b and a page past the end are refused; 1- and duplicates are honoured", async () => {
...
test.skip("opacity outside 0..1 and an unreadable colour are refused with nothing written", async () => {
...
test.skip("stdout carries JSON-RPC only, and the source makes no network call", async () => {
